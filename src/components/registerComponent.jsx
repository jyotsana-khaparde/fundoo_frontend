...
class RegisterComponent extends Component {
  ...
  handleSubmit = (event) => {
    ...
    if (this.state.fname === "") {
      ...
      toast("Confirm Email cannot be empty", {
        position: toast.POSITION.BOTTOM_CENTER,
      });
    } else if (
      !/[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,3}$/.test(this.state.confirmEmail)
    ) {
      toast("Invalid Confirm Email", {
        position: toast.POSITION.BOTTOM_CENTER,
      });
    } else if (this.state.username !== this.state.confirmEmail) {
      toast("Email and Confirm Email must be same", {
        position: toast.POSITION.BOTTOM_CENTER,
      });
      ...
      <div>
        <div id="regMainDiv">
          ...
          >
            ...
            <Button id="logoutButton" onClick={this.resetForm}>
              RESET
            </Button>
          </div>
          <ToastContainer />
